...
def case_make(duration_time, start_time, fault_t, fault_child_t, tpcc_t, cause_fault_number, effect_fault_number, cause_fault_time):
    ...
    thread1 = threading.Thread(target=execute_command,
                               args=(
                               f'python3 /root/mysqlrc/fault_injection/tpcc1.py -d {duration_time} -t {start_time} -i {fault_t} -k {fault_child_t} -s {tpcc_t} -c {cause_fault_number} -e {effect_fault_number} -p {path} -x {cause_fault_time}',))
    thread1.start()
    time.sleep(30)
