...
def case_make(duration_time, start_time, fault_t, fault_child_t, tpcc_t, cause_fault_number, effect_fault_number, cause_fault_time):
    ...
    thread1 = threading.Thread(target=execute_command,
                               args=(
                               f'python3 /root/mysqlrc/fault_injection/multi_anomaly.py -d {duration_time} -t {start_time} -i {fault_t} -k {fault_child_t} -s {tpcc_t} -c {cause_fault_number} -e {effect_fault_number} -p {path} -x {cause_fault_time}',))
    thread1.start()
    time.sleep(30)
...
def main(argv):
    ...
    try:
        # 处理传入的参数内容
        opts, args = getopt.getopt(argv, "hd:d:t:x:i:k:s:c:e:n:")
    ...
    for opt, arg in opts:
        if opt == '-d':
            duration = str(arg)
        if opt == '-t':
            fault_inject_time = str(arg)
        if opt == '-i':
            fault_type = "'" + str(arg) + "'"
        if opt == '-k':
            fault_child_type = "'" + str(arg) + "'"
        if opt == '-s':
            sleep_time = str(arg)
        if opt == '-c':
            total_cause_fault_number = str(arg)
        if opt == '-e':
            total_effect_fault_number = str(arg)
        if opt == '-n':
            total_data_number = int(arg)
        if opt == '-x':
            cause_inject_time = str(arg)

    for i in range(int(total_data_number)):
        print("start---------")
        case_make(duration, fault_inject_time, fault_type, fault_child_type, sleep_time, total_cause_fault_number,
                  total_effect_fault_number, cause_inject_time)
        time.sleep(120)
...
if __name__ == "__main__":
    ...
    # args = ['-d', '150', '-t', '60', '-x', '55', '-i', 'lock--->slow+lock+slow', '-k', 'record_lock+missing_index', '-s',
    #         '0.044', '-c', '7', '-e', '7', '-n', '1']
    # main(args)
    main(sys.argv[1:])
